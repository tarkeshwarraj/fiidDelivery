...
const MyOrders = () => {
  ...
  return (
    <div className="mt-16 pb-16">
      <div className="flex flex-col items-end w-max mg-8">
        <p className="text-2xl font-medium uppercase">My Orders</p>
        <div className="w-16 h-0.5 bg-primary rounded-full"></div>
      </div>

      {myOrders.map((order, index) => (
        <div key={index} className="border border-gray-300 rounded-lg mg-10 p-4 py-5 max-w-4xl" >
            ...
            {order.items.map((item, index)=>(
                <div key={index} className={`relative bg-white text-gray-500/70 ${order.items.length !== index + 1 && "border-b"} border-gray-300 flex flex-col md:flex-row md:items-center justify-between p-4 py-5 md:gap-16 w-full max-w-4xl`} >
                    ...
                    <div className="flex flex-col justify-center md:ml-8 mg-4 md:mb-0">
